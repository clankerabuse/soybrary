...
#
# keep_tokens=2 pins "soyjak, <variant>" so shuffle/dropout cannot drop the
# style lock. caption_dropout teaches the unconditional/empty prompt to stay
# on-distribution (soyjak), which is what stopped the previous run leaking
# generic SDXL images on soyjak-related tags.
[general]
