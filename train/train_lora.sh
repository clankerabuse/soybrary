#!/usr/bin/env bash
# train_lora.sh - Launch SDXL FULL fine-tune with kohya sd-scripts (sdxl_train.py).
# Name kept so existing cheat-sheets still work; this is no longer a LoRA run.
#
...
# OOM on 1× A100: BATCH_SIZE=1 GRAD_ACCUM_STEPS=16 MODE=full bash train/train_lora.sh
set -euo pipefail
...
# Defaults target 1× A100 40GB full fine-tune (Adafactor + grad checkpoint).
...
#
# keep_tokens=2 pins "soyjak, <variant>" as the SUBJECT of every image.
# Class-hijack tokens (1boy, portrait, wojak) are injected into every sidecar
# by ensure_trigger_captions.py so person/portrait prompts resolve to soyjaks.
# Extra object tags still train. caption_dropout_rate keeps the unconditional
# prior on soyjaks. Do NOT add a regularization/class image dir.
[general]
...
# --- Pin soyjak + class-hijack tokens on captions (idempotent) -------------
...
cd "$SDSCRIPTS_DIR"
echo "==> Launching SDXL FULL fine-tune (MODE=$MODE) — not LoRA"
...
echo "==> Training finished. Full SDXL checkpoint(s) written under /home/ubuntu/out"
echo "    (~6.5 GB each). Push to R2 with: MODE=$MODE bash train/push_model.sh"
