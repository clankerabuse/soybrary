...
# Extra tags (objects, settings, props) still train — they are allowed in
# the picture — but they cannot replace the soyjak. caption_dropout_rate is
# deliberately high so empty/unconditional steps stay on soyjaks too; that
# is what stops "pink_hair, tears" from becoming a photo of a real person.
[general]
...
# --- Pin soyjak trigger on captions (idempotent; needed for legacy shards) -
